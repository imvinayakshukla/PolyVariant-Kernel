...
class isInContourV3_Easy(Contour_Checking_fn):
    ...
    def __call__(self, pt):
        center = (float(pt[0] + self.patch_size // 2), float(pt[1] + self.patch_size // 2))  # Ensure center is a float
        if self.shift > 0:
            all_points = [
                (float(center[0] - self.shift), float(center[1] - self.shift)),
                (float(center[0] + self.shift), float(center[1] + self.shift)),
                (float(center[0] + self.shift), float(center[1] - self.shift)),
                (float(center[0] - self.shift), float(center[1] + self.shift))
            ]
        ...
        for points in all_points:
            # Ensure points are passed as floats
